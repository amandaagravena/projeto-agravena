...
Seleciona o banco de dados com os dados de SIF após a aplicação do
buffer. Divide os valores de latitude em 115 classes para representar
diferentes faixas latitudinais. Calcula o ponto médio de cada classe de
latitude, obtendo a latitude média de cada faixa. Agrupa os dados por
ano e latitude média. Calcula a média dos valores de SIF757 dentro de
cada combinação de ano e latitude. Remove os agrupamentos após o cálculo
das médias. Inicia a construção do gráfico utilizando os resultados
obtidos. Define a latitude média como eixo X e a média de SIF757 como
eixo Y. Cria boxplots para representar a distribuição dos valores de
SIF757 em cada faixa de latitude. Inverte os eixos para facilitar a
visualização das diferentes latitudes. Limita a escala dos valores de
SIF entre -0,3 e 0,6. Define os nomes dos eixos do gráfico. Aplica um
tema visual minimalista e remove a legenda. Utiliza a escala de cores
Viridis para diferenciar as classes de latitude.
...
df_sif_buffer <- df_sif_buffer |>
  mutate(
    sif_agr = daily_sif757 + 1.5 * daily_sif771
  )
```

# Conferir: coluna nova: sif_agr
...
df_sif_buffer |>
  ...
  labs(
    x = "Latitude (°)",
    y = "Concentração média de SIF composto"
  ) +
  theme_minimal() +
  theme(
    legend.position = "none"
